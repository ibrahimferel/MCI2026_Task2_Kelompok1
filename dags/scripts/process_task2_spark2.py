...
def run_spark_analytics():
    ...
    raw_data = df_raw.select(
        ...
        "add_to_cart_order", "reordered", "ingestion_timestamp"
    ).toPandas()
    ...
    # Step 1: Cast ke pandas nullable integer dulu
    for col in all_int_columns:
        raw_data[col] = raw_data[col].astype("Int64")

    # Step 2: Convert ke Python native int/None (aman dari numpy scalar)
    for col in all_int_columns:
    ...
    # DEBUG
    print(raw_data["days_since_prior_order"].head(20))
    print(raw_data["days_since_prior_order"].apply(type).head(20))

    # Step 3: to_dict tetap dipakai
    raw_data_tuples = [
    ...
    if raw_data_tuples:
        client.execute(
            'INSERT INTO analytics.raw_orders VALUES',
            raw_data_tuples,
            types_check=True
        )
    ...
    analytics_results["total_orders"] = analytics_results["total_orders"].apply(int)
    analytics_results["reorder_count"] = analytics_results["reorder_count"].apply(int)
    analytics_results["reorder_rate"] = analytics_results["reorder_rate"].apply(float)

    data_tuples = list(
        analytics_results.itertuples(index=False, name=None)
    )

    if data_tuples:
        client.execute(
            'INSERT INTO analytics.product_analytics VALUES', 
            data_tuples,
            types_check=True
        )
    ...
    hourly_results["total_orders"] = hourly_results["total_orders"].apply(int)

    hourly_tuples = list(
        hourly_results.itertuples(index=False, name=None)
    )

    if hourly_tuples:
        client.execute(
            'INSERT INTO analytics.hourly_analytics VALUES', 
            hourly_tuples,
            types_check=True
        )
    ...
    department_results["total_orders"] = department_results["total_orders"].apply(int)

    department_tuples = list(
        department_results.itertuples(index=False, name=None)
    )
