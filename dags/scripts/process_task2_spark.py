...
def run_spark_analytics():
    ...
    # Replace your existing raw_data block with this:
    ...
    raw_data = df_raw.select(
        ...
        "days_since_prior_order", "product_id", "product_name", "department",
        "aisle", "add_to_cart_order", "reordered", "ingestion_timestamp"
    ).toPandas()

    # Cast all columns to proper Python native types
    def safe_int(x):
        try:
            if pd.isna(x):
                return None
    ...
    raw_data_tuples = [
        (
            ...
            safe_int(row['order_number']),
            safe_int(row['order_dow']),
            safe_int(row['order_hour_of_day']),
            safe_int(row['days_since_prior_order']),  # Nullable, can be None
            safe_int(row['product_id']),
            safe_str(row['product_name']),
            safe_str(row['department']),
            safe_str(row['aisle']),
            safe_int(row['add_to_cart_order']),
            safe_int(row['reordered']),
            safe_str(row['ingestion_timestamp']),
        )
